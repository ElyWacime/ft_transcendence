...
curl -v http://localhost/api/dashboard/test-id 2>&1 | grep -E "HTTP|error|User"
echo ""

# Test 2: Check if endpoint is reachable
echo "Test 2: Check backend directly"
curl -v http://localhost:3000/api/dashboard/test-id 2>&1 | grep -E "HTTP|error|User"
echo ""

# Test 3: Check gateway
echo "Test 3: Check gateway health"
curl -v http://localhost/ 2>&1 | head -5
echo ""
